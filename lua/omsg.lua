...
The main representation of a message is a table with these elements:
    ...
    { headers   = list of header strings,
      ...
      sep       = EOL which, when doubled, separates the header from body, 
                  or the empty string if there's no separator (and no body),
      body      = string containing the original body (possibly empty),
...
local msg_meta = {
  ...
  __tostring = function(msg)
                 ...
                 local subject = header_tagged(msg, 'subject') or '<no subject>'
...
local function show_msg(what, m)
  ...
  for _, k in ipairs(util.table_sorted_keys(m.header_index)) do
    local v = m.header_index[k]
    eprintf('    %s = { %s }\n', k, table.concat(v, ', '))
  end
end
...
local new_only = {
  header_index = true,
  noncompliant = true,
  ['header_index.x-sendmail-from.1'] = true,
}
local diff_ok = {
  ['header_index.from '] = true,
  ['headers.1'] = true,
}

local function differ(v1, v2, pathname)
  pathname = pathname or ''
  if diff_ok[pathname] then
    return false
  elseif type(v1) ~= type(v2) then
    if type(v2) ~= 'nil' or not new_only[pathname] then
      eprintf('Field %s has type %s in fast message but type %s in old version\n',
              pathname, type(v1), type(v2))
      return true
    else
      return false
    end
  elseif type(v1) ~= 'table' then
    function cfstring(v1, v2)
      if type(v1) ~= 'string' then return ''
      else
        for i = 1, v1:len() do
          if v1:sub(i, i) ~= v2:sub(i, i) then
            return string.format(' (difference at position %d)', i)
          end
        end
        return string.format(' (second string is has extra suffix %s)',
                             show(v2:sub(v1:len()+1)))
      end
    end
    if v1 ~= v2 then
      eprintf('Fields %s differ in fast and old messages: %s vs %s%s\n',
              pathname, show(v1), show(v2), cfstring(v1, v2))
      return true
    else
      return false
    end
    ...
    local diff = false
    for k, nv1 in pairs(v1) do
      local np = pathname == '' and k or pathname .. '.' .. k
      if differ(nv1, v2[k], np) then
        diff = true
      end
    end
    for k in pairs(v2) do
      local np = pathname == '' and k or pathname .. '.' .. k
      if v1[k] == nil and not diff_ok[np] then
        eprintf('Field %s is missing from fast message but present in slow one: %s\n',
                np, show(v2[k]))
        diff = true
      end
    end
...
function of_string(s, uncertain)
  ...
  --eprintf('Old format header fields produces %s\n', show(header_fields))
  ...
  return setmetatable(msg, msg_meta)
end
 
local bug_compatible = true

local eols = { CRLF = '\r\n', LF = '\n', MIXED = '\n' }
local old_of_string = of_string
function of_string_fast(s, uncertain)
  local fm = require 'fastmime'
  local parsed = fm.parse(s)
--[[  eprintf('Output from parser is:\n')
  ...
  local sep = parsed.body and eol or ''
  if bug_compatible then sep = eol end
  local hi = util.table_tab { }
  for i = 1, #parsed.tags do
    table.insert(hi[parsed.tags[i]:lower()], i)
  end
  local msg = { headers = headers, header_fields = parsed.headerstring,
                noncompliant = parsed.noncompliant,
                body = parsed.body or '', sep = sep, eol = eol, header_index = hi,
                __parsed = parsed,
              }
  setmetatable(msg, msg_meta)
  if parsed.noncompliant then
    show_msg(string.format('Noncompliant message (%s):', parsed.noncompliant), msg)
  end
  return msg
end

function of_string_test(s, uncertain)
  local om = of_string(s, uncertain)
  local m  = of_string_fast(s, uncertain)
  --show_msg('Old message', om)
  --show_msg('Fast message', m)
  if differ(m, om) then
    if m.noncompliant then
      eprintf('Difference in noncompliant %s (%s)\n', tostring(m), m.noncompliant)
    else
      error(tostring(m) .. ' and ' .. tostring(om) .. ' differ')
    end
  end
  return m
end
...
This permits that folowing outputs of output.write may be done in a proper
way to be interpreted as the body of a message.
The header and boundary generated are comunicated to
util.set_output_to_message so output.write uses the same boundary for
next parts.
]]
