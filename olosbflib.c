...
#include "osbflib.h"
#else
enum foo { OSBF_CLOSED = 777 };
typedef struct class { enum foo state; const char *classname; } CLASS_STRUCT;
static void osbf_close_class (CLASS_STRUCT * class, lua_State *h);
static void osbf_close_class (CLASS_STRUCT * class, lua_State *h) {
  (void)class, (void)h;
}
#endif
...
static int lua_osbf_close_cached_classes(lua_State *L) {
  ...
  fprintf(stderr, "When closing caches, environment has type %s\n", lua_typename(L, lua_type(L, -1)));
  lua_getfield(L, -1, "cache");
  fprintf(stderr, "Indexing environment with key 'cache' produces %s\n",
          lua_typename(L, lua_type(L, -1)));
  lua_pop(L, 2);
#endif

  fprintf(stderr, "Trying the real thing...\n");
  lua_getfield(L, LUA_ENVIRONINDEX, "cache");  /* s: cache */
  fprintf(stderr, "Successfully indexed environment with key 'cache'\n");
  fprintf(stderr, "Walking through cache of type %s\n", lua_typename(L, lua_type(L, -1)));
  lua_pushnil(L);
...
static int lua_osbf_close_cache_and_exit(lua_State *L) {
  fprintf(stderr, "Closing caches before exit; top is %d\n", lua_gettop(L));
  lua_osbf_close_cached_classes(L);
  fprintf(stderr, "Getting ready to exit; top is %d\n", lua_gettop(L));
  lua_getfield(L, LUA_ENVIRONINDEX, "exit");  /* s: args exit */
  if (!lua_isfunction(L, -1)) { fprintf(stderr, "exit is not a function"); exit(1); }
  lua_insert(L, 1);             /* s: exit args */
  fprintf(stderr, "About to call; top is %d\n", lua_gettop(L));
  lua_call(L, lua_gettop(L), LUA_MULTRET);
  fprintf(stderr, "Exit returned %d results\n", lua_gettop(L));
  return lua_gettop(L);
}
...
OPENFUN (lua_State * L)
{
  ...
  /* push and set the shared environment, which can't be initialized yet */
  lua_newtable(L); 

#if SEGFAULT
  lua_pushvalue(L, 1);
  lua_replace(L, LUA_ENVIRONINDEX); /* s: env */

  /* make env.exit equal to os.exit */
  lua_getfield(L, LUA_GLOBALSINDEX, "os");      
  lua_getfield(L, -1, "exit");                         /* s: env os os.exit */
  lua_setfield(L, -3, "exit");                         /* s: env os */

  /* now set the new os.exit (which must have the current env) */
  lua_pushcfunction(L, lua_osbf_close_cache_and_exit); /* s: env os myexit */
  lua_setfield(L, -2, "exit");                         /* s: env os */
  lua_remove(L, -1);                                   /* s: env */
#else
  (void)lua_osbf_close_cache_and_exit;  /* avoid unused warning */
#endif

  /* set the cache in the environment */
  lua_newtable(L);              /* s: env cache */
  fprintf(stderr, "Setting environment cache to value of type %s\n",
          lua_typename(L, lua_type(L, -1)));
  lua_setfield(L, -2, "cache"); /* s: env */

#if !SEGFAULT
  lua_pushvalue(L, 1);
  lua_replace(L, LUA_ENVIRONINDEX); /* s: env */
  /* we're finished initializing the environment */
  lua_pop(L, 1);
  /*  lua_setfield(L, LUA_GLOBALSINDEX, "don't you touch me!!!"); */
#endif
